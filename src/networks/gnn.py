...
class GATModel(nn.Module):
    def __init__(
        ...
        if self.mode == 1:
            ...
            self.unwind_mlp = UnwindMLP(in_dim=1, out_dim=16)
            self.edge_type_proj = nn.Linear(1, edge_dim)
    ...
    def forward(self, x, node_unwind, edge_index, edge_attr, batch):
        ...
        if self.mode == 1:
            ...
            node_unwind_embed = self.unwind_mlp(node_unwind)
            x = torch.cat([x, node_unwind_embed], dim=1)
            edge_attr_proj = self.edge_type_proj(edge_type)
